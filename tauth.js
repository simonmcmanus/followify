...
var handleRequest = function() {
  console.log('IN HANDLE REQUEST', arguments);
};
  ...
  for(func in supported) {
    ...
    for(b = 0; b < items.length; b++ ){
      ...
      if(!current){ // new function.
        api[items[b]] = handleRequest.bind(this, key);
        current = api[items[b]];
      }else { // api is defined and current set
        current[items[b]] = handleRequest;
 ...
 *
 * @param  {Number}   twitterId       The handle you want the authenticated user to follow.
 ...
 */
self.follow = function(twitterId, oauthToken, oauthTokenSecret, callback) {
  var body = {
    follow: true,
    user_id: twitterId
  };
  this.post('https://api.twitter.com/1.1/friendships/create.json', oauthToken, oauthTokenSecret, body, callback);
};
...
self.friendships = {
  lookup: function(handles, oauthToken, oauthTokenSecret, callback) {
    var processData = function(error, data, limit) {
      callback(error, {
        limit: limit,
        user: data
    ...
    self.fetch('https://api.twitter.com/1.1/friendships/lookup.json?screen_name='+handles.join(','), oauthToken, oauthTokenSecret, processData);
  },
  incoming: function(oauthToken, oauthTokenSecret, callback) {
    var processData = function(error, data, limit) {
      callback(error, {
        limit: limit,
        user: data
    ...
    self.fetch('https://api.twitter.com/1.1/friendships/incoming.json', oauthToken, oauthTokenSecret, processData);
  }
};
